...
void lowerEL_sync_interrupt()
{
	...
	if((esr >> 26) == 0b010101) // SVC instruction execution in AArch64 state.
	{
        ...
        else if (svc == 1)  // get pid
		{
            unsigned long pid = get_pid();
            set_x0(pid);
        }
		else if (svc == 2) // uart read
		{
            unsigned long ret = uart_gets((char*)x0,(int)x1);
			set_x0(ret);
        }
		else if (svc == 3) // uart write
		{
            uart_putstr((char*)x0);
            set_x0(x1);
        }
		...
		{
			struct file *file = vfs_open((char *)x0, (int)x1);	
			int fd = thread_register_fd(file);
            set_x0(fd);
		}
		else if (svc == 8) // close
		{
			struct file *file = thread_get_file((int)x0);
			thread_clear_fd((int)x0);
		...
		{
			struct file *file = thread_get_file((int)x0);
			const void *buf = (const void *)x1;
			unsigned int len = (unsigned int)x2;
			unsigned int size = vfs_write(file, buf, len);
			set_x0(size);
		}
		else if (svc == 10) // read
		{
			...
			void *buf = (void *)x1;
			unsigned int len = (unsigned int)x2;		
			unsigned int size = vfs_read(file, buf, len);
			set_x0(size);
		}
		else if (svc == 11) // list
		{
			...
			void *buf = (void *)x1;
			int index = (int)x2;		
			unsigned int size = vfs_list(file, buf, index);
			set_x0(size);
		}
		else if (svc == 12) // mkdir
		{
			int result = vfs_mkdir((const char *)x0);
			set_x0(result);
		}
		else if (svc == 13) // chdir
		{
			int result = vfs_chdir((const char *)x0);
			set_x0(result);
		}
		else if (svc == 14) // mount
		{
			const char *device = (const char *)x0;
			const char *mountpoint = (const char *)x1;
			const char *filesystem = (const char *)x2;
			int result = vfs_mount(device, mountpoint, filesystem);
			set_x0(result);
		}
		else if (svc == 15) // unmount
		{
			int result = vfs_umount((const char *)x0);
			set_x0(result);
		}
	}
	else if ((esr >> 26) == 0b100100)
	{
		...
		char buf[16] = {0};
		uart_putstr("[Data Abort] pid: ");
		unsignedlonglongToStr(get_current()->tid, buf);
		uart_putstr(buf);
		uart_putstr(", far_el1: ");
		unsignedlonglongToStrHex(far, buf);
		uart_putstr(buf);
		uart_putstr("\n");
		// 終止目前process
		exit();
