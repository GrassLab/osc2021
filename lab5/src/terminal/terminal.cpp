...
void Terminal::TriggerException() {
    Kernel::Syscall(Sys::PRINT_EXECPTION_REG);
}

void Terminal::StartTimer() {
    Kernel::Syscall(Sys::TIMER_ENABLE);
}

void Terminal::StopTimer() {
    Kernel::Syscall(Sys::TIMER_DISABLE);
}

void thread() {
    uint64_t thread_id = Kernel::Syscall(Sys::GET_THREAD_ID);
    for (int i = 0; i < 49 * thread_id % 12; i++) {
        Kernel::Delay(1000000);
        IO() << "Current thread id: " << Kernel::Syscall(Sys::GET_THREAD_ID) << "\r\n";
        Kernel::Syscall(Sys::SCHEDULE);
    }
}

void Terminal::TestThread() {
    for (int i = 0; i < 10; i++) {
        Kernel::Syscall(Sys::CREATE_THREAD, (uint64_t)thread);
    }
    while (Kernel::totalThreads > 1) {
        Kernel::Syscall(Sys::SCHEDULE);
    }
}
