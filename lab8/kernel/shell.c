...
struct Command {
    ...
    {.name = "lab8", .func = lab8, .usage = "lab8"},
...
static void user_test(){
    ...
    exec("initramfs/argv_test", argv);
    //exec("preempt_test_delay", NULL);
}
...
static void lab8_test(){
    exec("initramfs/lab8_test", NULL);
}

static void lab8(){
    thread_create(lab8_test);
    idle();
}

static void test_code(){
    printf("%p" NEW_LINE, lab8);
