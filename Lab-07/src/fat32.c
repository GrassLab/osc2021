...
int fat32_setup_mount(struct filesystem* fs, struct mount* mount) {
    ...
    for(int i = 0; dir[i].name[0] != '\0'; i++ ) {
		if(dir[i].name[0]==0xE5 || dir[i].attr[0]==0xF) //0x00 Entry never used, 0xe5 File is deleted
            continue;
...
void memory_dump() {
    unsigned long a,b;
    for(a=(unsigned long)0;a<(unsigned long)512;a+=16) {
        printf("%05x",(unsigned long)a); printf(": ");
        for(b=a ; b<a+16 ; b+=2) {
