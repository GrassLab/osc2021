...
int path(char* input)
{
    ...
    if(!tabel_cpio)
    {
        ...
        {   
            ...
            first = 0;
            f[number_of_file].data_size = file_size;
            char *filename = kernel + 0x6E;
            char *data = kernel + name_size;
            f[number_of_file].file_addr = filename;
            f[number_of_file].data_addr = data;
...
void dtb()
{
    ...
    header.magic = my_atoi(kernel,4);
    kernel = kernel + 4;
    header.totalsize = my_atoi(kernel,4);
    kernel = kernel + 4;
    header.off_dt_struct = my_atoi(kernel,4);
    kernel = kernel + 4;
    header.off_dt_strings = my_atoi(kernel,4);
    kernel = kernel + 4;
    header.off_mem_rsvmap = my_atoi(kernel,4);
    kernel = kernel + 4;
    header.version = my_atoi(kernel,4);
    kernel = kernel + 4;
    header.last_comp_version = my_atoi(kernel,4);
    kernel = kernel + 4;
    header.boot_cpuid_phys = my_atoi(kernel,4);
    kernel = kernel + 4;
    header.size_dt_strings = my_atoi(kernel,4);
    kernel = kernel + 4;
    header.size_dt_struct = my_atoi(kernel,4);
    kernel = kernel + 4;
