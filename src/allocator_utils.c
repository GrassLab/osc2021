...
struct FrameListNum freeListElement[MAX_ELEMENT_NUM];       /*used to construct freelist*/
...
// Chunk Functions
struct FrameChunk* _new_chunk_from_idx(uint32_t index, struct FrameChunk *next, struct FrameChunk *prev){       //create a new chunk structure, and concate to last
...
struct FrameChunk* _new_chunk_from_zero(struct FrameChunk *prev){
    uint64_t base_addr = _new_frame(0, 0, _find_capable_slot_size(0), 1);   //allocate a single frame bage, which is used to cut into chunk
    uint32_t index = _get_index_from_mem(frame_array->base_addr, base_addr);    //get the index of memory block address
