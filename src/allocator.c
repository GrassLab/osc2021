...
int16_t _cal_log_2(uint64_t num){   //calculate log2 of num
    int16_t res = 0;
    while(num){
        num >>= 1;
        res += 1;
...
struct FrameListNum* _get_last_list_element(struct FrameListNum *header){   //return the last item of freelist
    while(header->next)
        header = header->next;
    return header;
}

struct FrameListNum* _get_new_list_element(uint32_t index, struct FrameListNum *next, struct FrameListNum *prev){   //create a freelist element and put next and prev in
    cur_element_idx += 1;
...
void _insert_to_frameList(struct _RawFrameArray *frame_array, uint16_t power_idx, uint32_t element_idx){    //insert to the last position of freelist[power_idx]
    struct FrameListNum *cursor;
    if(!frame_array->freeList[power_idx]){  //if null,(current freelist of power_idx is empty)
...
uint32_t _allocate_slot(struct _RawFrameArray *frame_array, uint64_t need_size, int16_t need_size_power, int16_t find_size_power){  //need_size => num of needed block
    uint32_t index = frame_array->freeList[find_size_power]->index;
    uint32_t return_addr = frame_array->base_addr + (4096 * (uint32_t)index);           //returned address (starting address of required mem)
    frame_array->freeList[find_size_power] = frame_array->freeList[find_size_power]->next;  //remove allocated one from freelist
    ...
    if(find_size_power != need_size_power){
        ...
        while(cur_size_power >= need_size_power){   //put right bottom left to freelist 
    ...
    for(;cur_index<cur_end_index; cur_index++)
        frame_array->val[cur_index] = UNFREE_SLOT;  //write to val of allocated block
...
void _merge_free_list(struct _RawFrameArray *self){
    ...
    for(i=0; i<20; i++){
        cur_base = 1<<(i+1);
        left = self->freeList[i];       //merging from bottom to up
        while(left){
            right = left->next;
            while(right){
                if((left->index^right->index) < cur_base){      //xor with the range of curbase, then we know that we can merge these two head together
                    ...
                    uart_puts("\r\n");
                    if(left->next == right)     //useless, boost only
                        left = left->next;
                    break;
                }
                right = right->next;
...
void free_memory(struct _RawFrameArray *self, uint64_t free_addr, uint64_t free_size){
    free_size /= 0x1000;    //blocks
    int16_t free_size_power = _cal_log_2(free_size);
    uint32_t index = (free_addr - self->base_addr) >> 12;   // divide by 4096, starting index
    uint32_t cur_index;
    uint32_t free_size_power_len = 1<<free_size_power;

    if(free_size+1 == free_size_power_len){ //number of released block is a power of 2
    ...
    else if(free_size < free_size_power_len){   //there are redundant blocks before
...
uint32_t new_memory(struct _RawFrameArray *self, uint64_t need_size){
    ...
    while(1){
        ...
        if(self->freeList[find_size_power])
            break;
        find_size_power += 1;   //search for empty item from tight to loose
    }
    return _allocate_slot(self, need_size, need_size_power, find_size_power);   //allocate real size
}

FrameArray* NewFrameArray(){    //initialization
