...
static size_t startup_allocator(uint32_t num_of_page, uint32_t *mem_node, uint32_t mem_node_size){
    size_t return_index = 1;
    uint32_t found = 0;
    while(1){
        size_t iii;
        for(iii=0; iii<num_of_page; iii++){
            size_t find_index = return_index + iii;
            if(find_index == TOTAL_NUM_OF_PAGE){
                miniuart_send_S("Error: No page for startup allocator!!!" NEW_LINE);
                Infinite_Loop(1);
            }
            size_t res_mem_index_start = (((uint64_t)bswap32(mem_node[mem_node_size-2]) + (uint64_t)bswap32(mem_node[mem_node_size-1])) & ~(0xFFFU)) / PAGE_SIZE;
            ...
            for(int64_t i=(int64_t)mem_node_size-4; i>=0; i-=2){
                res_mem_index_start = (((uint64_t)bswap32(mem_node[i]) + (uint64_t)bswap32(mem_node[i+1])) & ~(0xFFFU)) / PAGE_SIZE;
                res_mem_index_end = (((uint64_t)bswap32(mem_node[i+2]) + 0xFFFU) & ~(0xFFFU)) / PAGE_SIZE;;
...
void buddy_init(){
    ...
    size_t buddy_system_size = (sizeof(uint16_t) * (TOTAL_NUM_OF_PAGE >> 3)) + (sizeof(Page *) * MAX_ORDER);
    buddy_system_size = ((buddy_system_size + 0xFFFU) & ~(0xFFFU)) / PAGE_SIZE;
    size_t startup_alloc_index = startup_allocator(buddy_system_size, value, value_count);
    size_t startup_alloc_p = BUDDY_START_ADDR + (startup_alloc_index * PAGE_SIZE);
    printf("buddy_system_size = %ld pages, startup_alloc_index = %ld" NEW_LINE, buddy_system_size, startup_alloc_index);

    buddy.page_frame_bit = (void *)startup_alloc_p;
    buddy.free_page_list = (void *)startup_alloc_p + (sizeof(uint16_t) * (TOTAL_NUM_OF_PAGE >> 3));
    ...
    for(size_t i=startup_alloc_index; i<startup_alloc_index+buddy_system_size ; i++){
    ...
    printf("res mem from %ld to %ld" NEW_LINE, res_mem_index_start, res_mem_index_end);
    ...
    printf("kernel from %ld to %ld" NEW_LINE, BUDDY_START_ADDR, kernel_index_end);
    ...
    if(INITRAMFS_ADDR != 0){
        ...
        printf("ramfs from %ld to %ld" NEW_LINE, initramfs_index_start, initramfs_index_end);
    }
    ...
    if(bswap32(blk->magic) == DTB_MAGIC_NUM){
        ...
        printf("dtb from %ld to %ld" NEW_LINE, dtb_index_start, dtb_index_end);
    }
...
void *malloc(size_t num_of_byte){
    ...
    if(num_of_byte > sizeof(((Dynamic_Mem_Page *)0)->data)){
        miniuart_send_S("Warning: Exceed malloc size, using page allocator instead" NEW_LINE);
        return palloc((num_of_byte + PAGE_SIZE) / PAGE_SIZE);
    }
