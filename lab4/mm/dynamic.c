...
void* dynamic_malloc(size_t size) {
  ...
  //bin size should be larger than 0x20 header size
  ...
  //should initialize, but I'm lazy
...
void dynamic_free(void* address) {
  ...
  if(chunk->size >= 0x20 && chunk->size <= DYNAMIC_BIN_MAX * DYNAMIC_BIN_MIN_SLOT) {
    ...
    if(chunk->prev_size >= 0x20 && (prev_chunk->size & 0x1) == 0) {
      //prev chunk is freed
...
void dynamic_top_chunk_free() {
  ...
  if(size >= DYNAMIC_BIN_MIN_SLOT * 2) {
