...
int dma_get_slot_no(int mbytes){
  if (mbytes <= BUDDY_DMA_LEVEL_THRE){
    return round_up(mbytes, BUDDY_DMA_UNIT_SIZE);
  }
  else{
    int t = BUDDY_PAGE_SIZE/mbytes;
    return (16-t)+BUDDY_DMA_LEVEL_THRE/BUDDY_DMA_UNIT_SIZE;
  }
}

int dma_get_chunk_size(int slot_no){
  if (slot_no <= BUDDY_DMA_LEVEL_THRE/BUDDY_DMA_UNIT_SIZE){
    return slot_no*BUDDY_DMA_UNIT_SIZE;
  }
  else{
