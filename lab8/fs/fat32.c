...
void set_dir_entry(struct directory_entry* d_entry, const char* component_name, size_t cluster_num) {
  ...
  for(int i = 0; i < FAT32_D_ENTRY_EXTENSION_SIZE; i++) {
    d_entry->extension[i] = '\x20';
  }

  for(int j = 0; j < strlen(component_name); j++) {
    if(j >= FAT32_D_ENTRY_NAME_SIZE) 
      break;
    
    if(component_name[j] == '.') {
      j++;
      for(int k = 0; (k + j) < strlen(component_name); k++) {
        if(k >= FAT32_D_ENTRY_EXTENSION_SIZE) 
          break;

        d_entry->extension[k] = component_name[k + j];
      }
      break;
    }
    d_entry->name[j] = component_name[j];
  }
    

  d_entry->attribute = 0x20;
  d_entry->start_cluster_high = (cluster_num >> 16);
  d_entry->start_cluster_low = ((cluster_num << 16) >> 16);
  d_entry->size = 0;
...
static int create(struct vnode* dir_node, struct vnode** target, const char* component_name) {
  ...
  struct vnode* v_node;
  struct directory_entry *d_entry;
  size_t cluster_num, new_cluster_num, pre_cluster_num;
  char fat_buf[FAT32_BLOCK_SIZE], fat_table[FAT32_BLOCK_SIZE];
  int i;
  ...
  new_cluster_num = fat32_find_free_cluster(inode);
  
  if(new_cluster_num == -1)
    return 1;
  
  //find empty directory entry in diectory table
  cluster_num = (d_entry->start_cluster_high << 16) + d_entry->start_cluster_low;
  pre_cluster_num = cluster_num;
  ...
  while(1) {
    ...
    d_entry = (struct directory_entry *)fat_buf;
    i = 0;
    //printf("cluster_num: %d\n", cluster_num);
    //traverse directory table
    while(d_entry + i <= fat_buf + FAT32_BLOCK_SIZE) {
      if(*(char*)(d_entry + i) == '\x00') {
        //dir end
        
        if(d_entry + i + 1 > fat_buf + FAT32_BLOCK_SIZE) {
          //need next cluster to store d_entry table
          pre_cluster_num = cluster_num;
          cluster_num = fat32_find_free_cluster(inode);
          ...
          readblock(inode->lba + inode->num_of_reserved_sectors + pre_cluster_num / FAT32_ENTRY_PER_FAT_TABLE, fat_table);
          *(uint32_t *)(fat_table + (pre_cluster_num % 128) * 4) = cluster_num;
          ...
          // set as last enry
          *(char*)(d_entry + i + 1) = '\x00';
           
        }
        //add at the end of directory table
        set_dir_entry(d_entry + i, component_name, new_cluster_num);
        //set next d entry as last entry
        
        writeblock(inode->lba + inode->num_of_reserved_sectors + inode->sectors_per_fat_large_fat32 * 2 + cluster_num - 2, fat_buf);
        
        v_node = fat32_vnode_create(dir_node->mount, d_entry + i, cluster_num, i);
