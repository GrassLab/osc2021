...
#define CPIO_BASE ((struct cpio_newc_header*)0x8000000)
...
char* fdataGet(void* _addr,unsigned long* size){
  struct cpio_newc_header* addr=(struct cpio_newc_header*)_addr;
  unsigned long psize=strToU(addr->c_namesize),dsize=strToU(addr->c_filesize);
  *size=dsize;
  if((sizeof(struct cpio_newc_header)+psize)&3)psize+=4-((sizeof(struct cpio_newc_header)+psize)&3);
...
void* nextfGet(void* _addr){
  ...
  addr=(struct cpio_newc_header*)(data+dsize);
  return addr;
}
