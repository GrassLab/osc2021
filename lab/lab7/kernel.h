...
int fatfs_write(struct file *, void *, size_t);
int fatfs_read(struct file *, void *, size_t);
int fatfs_lookup(struct vnode *, struct vnode **, char *);
int fatfs_mount(struct filesystem *, struct mount *);
int fatfs_create(struct vnode *, struct vnode **, char *);
int fatfs_list(struct file *file, char *buff, int i);
...
int register_filesystem(struct filesystem *fs) {
  ...
  if (strcmp(fs->name, "tmpfs")) {
    ...
    // fatfs_f_ops.create = fatfs_create;
...
#define FAT_BUF_SIZE SECTOR_SIZE
/*
struct mbr_entry {
  char status;  // partition status: none activated: 0x00 ; activated: 0x80
  char start_chs[3];  // partition head number and partition sector number  * 2
  char partition_type;  // FAT32: 0x0B, FAT16: 0x04, NTFS: 0x07
  char end_chs[3];
  unsigned int addr;  // filesystem entry address
...
struct mbr {
  ...
  /* logic addr: 0x1BE; Partition table; table size: 16 bytes; 4 partition */
  struct mbr_entry entry[4];
  char signature[2];
} __attribute__((packed));

struct fat_BS {
  /* Boot Sector; 11 bytes */
  unsigned char bootjmp[3];
  unsigned char oem_name[8];

  /* DOS 2.0 BIOS Parameter Block; 13 bytes */
  unsigned short bytes_per_sector;
  unsigned char sectors_per_cluster;  // Logical sectors per cluster
  /* Count of reserved logical sectors,
   * The number of logical sectors before the first FAT in the file system image
   */
  unsigned short reserved_sector_count;
  unsigned char table_count;  // Number of File Allocation Tables
  ...
  /* FAT32 Extended BIOS Parameter Block; 54 bytes */
  unsigned int table_size_32;  // Logical sectors per file allocation table
  unsigned short extended_flags;
  unsigned short fat_version;
  unsigned int root_cluster;  // Cluster number of root directory at start
 ...
 */
struct fat_root {
  unsigned char filename[8];   // Short file name
  unsigned char extension[3];  // Short file extension
  unsigned char file_attr;
  unsigned char reserved;
  unsigned char created_stamp;  // Create time
  unsigned short created_time;
  unsigned short created_date;
  unsigned short last_date;  // Last access date
  ...
  unsigned short cluster_l;  // Entries with the Volume Label flag
...
struct fatentry {
  char name[15];
  unsigned int cluster_id;  // (fat_root->cluster_h << 2) + fat_root->cluster_l
  FILE_TYPE type;
  int name_len;
  struct vnode *vnode;
...
int fat_base, root_sector;
int first_data_sector, sectors_per_cluster;
struct fat_root *fat_root;

/*
 * Cluster number of root directory start,
 * typically 2 (first cluster) if it contains no bad sector.
 * typically, cluster sizes range from 2 to 32 KiB
 * So, basically first_sector just equal to first_data_sector
 */
int get_first_sector(int cluster, int _sectors_per_cluster,
                     int _first_data_sector) {
  return ((cluster - 2) * _sectors_per_cluster) + _first_data_sector;
}

void init_fatentry(struct fatentry *fatentry, struct vnode *vnode, char *name) {
  fatentry->vnode = vnode;
  strcpy(fatentry->name, name);
}
void fatfs_strcpy(char *destination, char *source, int len) {
  int i = 0;
  /* filename may use space to padding filename to len = 8 */
  for (; source[i] != ' ' && i < len; ++i) destination[i] = source[i];
  destination[i] = '\0';
}
int fatfs_mount(struct filesystem *fs, struct mount *mount) {
  struct mbr *mbr = kmalloc(SECTOR_SIZE);
  readblock(0, (char *)mbr);
  fat_base = mbr->entry[0].addr;
  // asm volatile("mbr:");

  struct fat_BS *fat_boot = kmalloc(SECTOR_SIZE);
  ...
  first_data_sector = fat_boot->reserved_sector_count +
                      fat_boot->table_count * fat_boot->table_size_32;
  unsigned int root_cluster_32 = fat_boot->root_cluster;

  root_sector = get_first_sector(root_cluster_32, sectors_per_cluster,
                                 first_data_sector) +
                fat_base;
  fat_root = kmalloc(SECTOR_SIZE);
  readblock(root_sector, (char *)fat_root);

  /* init vnode, mount fat and fatentry*/
  mount->fs = fs;
  ...
  for (int i = 0; i < DIR_MAX; i++) {
    ...
    /* copy information and built fatentry with fat_root */
    if ((fat_root + i)->filename[0]) {
      /* copy filename.extension from fat_root to fatentry */
      fatfs_strcpy(fatentry->list[i]->name, (fat_root + i)->filename, 8);
      size_t len = strlen(fatentry->list[i]->name);
      fatentry->list[i]->name_len = len;
      *(fatentry->list[i]->name + len) = '.';
      fatfs_strcpy(fatentry->list[i]->name + len + 1, (fat_root + i)->extension,
                   3);

      fatentry->list[i]->cluster_id =
          ((fat_root + i)->cluster_h << 2) + (fat_root + i)->cluster_l;

      /* buffer in fatentry buf is still emtpy, load in as call read */
      fatentry->list[i]->buf = kmalloc(sizeof(struct fatfs_buf));
      fatentry->list[i]->buf->size = (fat_root + i)->file_size;

      struct vnode *vnode_child = (struct vnode *)kmalloc(sizeof(struct vnode));
...
int fatfs_lookup(struct vnode *dir_node, struct vnode **target,
                 char *component_name) {
  struct fatentry *dir_entry = (struct fatentry *)dir_node->internal;
  for (int i = 0; i < DIR_MAX; i++) {
    if (strcmp(dir_entry->list[i]->name, component_name)) {
      struct fatentry *fatentry = dir_entry->list[i];
...
int fatfs_read(struct file *file, void *buf, size_t len) {
  struct fatentry *fat_entry = (struct fatentry *)file->vnode->internal;
  int index = get_first_sector(fat_entry->cluster_id, sectors_per_cluster,
                               first_data_sector) +
              fat_base;
  readblock(index + file->f_pos / SECTOR_SIZE, fat_entry->buf->buffer);
  size_t i = 0;
  for (; i < len; ++i) {
    ((char *)buf)[i] = fat_entry->buf->buffer[i];
    if (++(file->f_pos) >= fat_entry->buf->size) break;
  }
  return i;
}

int fatfs_write(struct file *file, void *buf, size_t len) {
  struct fatentry *fatentry = (struct fatentry *)file->vnode->internal;
  size_t i = 0;
  for (; i < len; ++i, file->f_pos++) {
    if (i >= FAT_BUF_SIZE) {
      printf("fatfs buffer error: over buffer limit\n");
      return -1;
    }
    fatentry->buf->buffer[file->f_pos % FAT_BUF_SIZE] = ((char *)buf)[i];
  }
  fatentry->buf->size = file->f_pos;
  int index = get_first_sector(fatentry->cluster_id, sectors_per_cluster,
                               first_data_sector) +
              fat_base;
  writeblock(index, fatentry->buf->buffer);

  /* update filesize in direct entry*/
  readblock(root_sector, (char *)fat_root);
  for (int j = 0; j < DIR_MAX; ++j) {
    char filename[15];
    fatfs_strcpy(filename, (fat_root + j)->filename, 8);
    size_t len = strlen(filename);
    filename[len] = '.';
    fatfs_strcpy(filename + len + 1, (fat_root + j)->extension, 3);
    if (strcmp(filename, fatentry->name)) {
      (fat_root + j)->file_size = fatentry->buf->size;
      break;
    }
  }
...
int fatfs_list(struct file *file, char *buff, int i) {
  struct fatentry *dir = (struct fatentry *)file->vnode->internal;
  if (dir->type == FILE_TYPE_D) {
    if (dir->list[i]->type != FILE_TYPE_N)
      memcpy(buff, dir->list[i]->name, strlen(dir->list[i]->name) + 1);
    return dir->list[i]->buf->size;
