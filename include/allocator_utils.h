...
// FreeList Functions
int32_t _find_capable_slot_size(int32_t);
...
void _new_frameList_element(int32_t, uint32_t);
void _rm_frameList_element(struct FrameListNum*, int);
void _merge_frameList_element();

// Chunk Functions
struct FrameChunk* _new_chunk_from_idx(uint32_t, struct FrameChunk*, struct FrameChunk*);
struct FrameChunk* _new_chunk_from_zero(struct FrameChunk*);
