...
void read_data_region(int cluster_num, void* buf) {
    ...
    // fat32_meta.root_cluster_num default is 2
    // add 2048 later 
}
...
unsigned int get_fat_entry(int cluster_num) { // ??
    ...
    read_partition_block(fat32_meta.num_reserved_sector + cluster_num / (512/4), (unsigned char *)fat);
...
void set_fat_entry(int cluster_num, unsigned int val) { // ??
    ...
    write_partition_block(fat32_meta.num_reserved_sector + cluster_num / (512/4), (unsigned char *)fat);
}
...
int fat32_write(file* file, const void* buf, unsigned long len) {
    ...
    // todo: if file size == 0, allocate a cluster for it
    if(content->size == 0) {
    ...
    for(; len_write < len; f_pos++, len_write++) {
        ...
        if(++iter % 512 == 0) {
            ...
            iter = 0;
            cluster_index = get_fat_entry(cluster_index);
            if(cluster_index == EOC) { // reach the last cluster in file, we should extend new cluster for it
...
int fat32_init(mount* mnt, vnode* root) {
    uart_puts("init fat32\n");
    unsigned char buf[BLOCK_SIZE];
    read_partition_block(0, buf); // add 2048 later
    uart_puts("read fat32 metadata\n");
    fat32_meta.bytes_per_sector = *((unsigned short *)(buf+0x00b));
    ...
    uart_printf("bytes per sector: %d\n", fat32_meta.bytes_per_sector);
    uart_printf("sectors per cluster: %d\n", fat32_meta.sectors_per_cluster);
