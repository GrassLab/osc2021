...
void* updatePT(void* page_table0,void* va){//all address in table are physical address
	...
	int index[4];//virtual address index
	tmp>>=12;
	index[3]=tmp&0x1ff;
	tmp>>=9;
	index[2]=tmp&0x1ff;
	...
	for(int i=0;i<=2;++i){//trace page table
		if(table[index[i]]==0){//if page not initialized
	...
	table[index[3]]=VA2PA(frame)|(1<<10)|(1<<6)|MAIR_IDX_NORMAL_NOCACHE<<2|0b11;//for user/kernel access
	return frame;
}
