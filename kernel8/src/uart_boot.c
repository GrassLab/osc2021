...
extern char __prog_start[];
extern char __prog_end[];


void loadimg(){
	uart_puts("Copy bootloader to 0x60000\r\n");
    char *current_addr = __prog_start;
    char *end_addr = __prog_end;
    ...
    void (*function_call)(void) = (void (*)(void))(func_addr - (unsigned long int)__prog_start + TEMP_ADDR);
    function_call();
