...
#define CPIO_BASE  0x8000000 + KVA
...
void cpio_list() 
{
	...
	unsigned long long ptr = CPIO_BASE;
...
{
	unsigned long long ptr = CPIO_BASE;  // cpio address
	unsigned long long loadAddr = 0x4000000 + KVA;
...
{
	uart_putstr("===== Populate the root file system with initramfs =====\n");
	unsigned long long ptr = CPIO_BASE;
	struct cpio_newc_header *header;
	char buf[16] = {0};
	...
	while (1) 
	{
		...
		char* pathname = (char*)(ptr + sizeof(struct cpio_newc_header));	
		if (strcmpn(pathname, CPIO_TRAILER, 10))
			return;

		unsigned long namesize = hex2int(header->c_namesize, 8);
		unsigned long filesize = hex2int(header->c_filesize, 8);

		uart_putstr("pathname: ");
		uart_putstr(pathname);
		uart_putstr(", filesize: ");
		unsignedlonglongToStr(filesize, buf);
		uart_putstr(buf);
		uart_putstr("\n");

		ptr = (unsigned long long)pathname;
		ptr = align_to_4(ptr + namesize);
