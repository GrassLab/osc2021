...
#define BOOT_PUD_ATTR_NOR (PD_ACCESS | (MAIR_IDX_NORMAL_NOCACHE << 2) | PD_BLOCK)//for L1 (ARM peripherals)
...
void initMMU(){//be careful when using stack memory!
	...
	//L1 table init
//	asm volatile("str %0, [%1]\n"::"r"(0x00000000|BOOT_PGD_ATTR),"r"(0x1000));//finer granularity for different memory type
