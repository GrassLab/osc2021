...
extern void set_ttbr0_el1(uint64_t ttbr0_el1);
...
void svc_handler(uint64_t *trap_frame){
    ...
    //set_ttbr0_el1((uint64_t)current_thread()->pgd);
}

void page_fault_handler(void *far_el1){
    printf("Error: Page fault at %p!!!" NEW_LINE, far_el1);
}
